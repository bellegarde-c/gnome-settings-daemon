...
#define GSD_LINEAR_BRIGHTNESS_RAMP 10
#define GSD_LINEAR_BRIGHTNESS_SUN_LUX 70000
...
iio_proxy_changed (GsdPowerManager *manager)
{
        ...
        manager->linear_brightness_ambient = MIN (GSD_LINEAR_BRIGHTNESS_SUN_LUX,
                        MAX (0, manager->linear_brightness_ambient_iio +
                                g_settings_get_double(manager->settings,
                                                      "ambient-normalize")));

        g_debug ("Read last light level: %f", manager->linear_brightness_ambient);

        if (manager->linear_brightness_ambient_unit == GSD_POWER_LIGHT_LEVEL_UNIT_VENDOR) {
                gsd_backlight_set_brightness_async (manager->backlight,
                                                    manager->linear_brightness_ambient,
                                                    NULL, NULL, NULL);
        } else if (manager->linear_brightness_ambient > manager->linear_brightness_ambient_up ||
                        manager->linear_brightness_ambient <= manager->linear_brightness_ambient_down) {
...
set_linear_brightness (GsdPowerManager *manager)
{
        ...
        g_debug("linear brightness: %f -> %d",
                manager->linear_brightness_ambient, manager->linear_brightness);
...
get_linear_brightness (GsdPowerManager *manager)
{
        ...
        while (g_variant_iter_loop (iter, "{ii}", &light_level, &brightness_level)) {
                ...
                if ((gint) manager->linear_brightness_ambient <= light_level) {
                        ...
                        return (
                                ((gint) manager->linear_brightness_ambient - manager->linear_brightness_ambient_down) *
...
static gdouble
get_ambient_for_linear_brightness (GsdPowerManager *manager, gint brightness)
{
        g_autoptr (GVariantIter) iter;
        gint light_level;
        gint brightness_level;
