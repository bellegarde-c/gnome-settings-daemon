...
#define GSD_LINEAR_BRIGHTNESS_DELAY_DOWN 2500
/* Apply brightness at this rate */
#define GSD_LINEAR_BRIGHTNESS_RAMP       25
/* Delay ramp down once this value has been applied */
#define GSD_LINEAR_BRIGHTNESS_RAMP_DOWN  5
...
iio_proxy_changed (GsdPowerManager *manager)
{
        ...
        manager->linear_brightness_ambient_iio = g_variant_get_double (val_als);
        ...
        linear_brightness_ambient = MAX (1, manager->linear_brightness_ambient_iio +
                                            g_settings_get_int(manager->settings,
                                                                  "ambient-light-normalize"));

        /* Ambient light moved to upper range */
        if (linear_brightness_ambient > manager->linear_brightness_ambient_up) {
                manager->linear_brightness_ambient = linear_brightness_ambient;
        ...
        } else if (linear_brightness_ambient <= manager->linear_brightness_ambient_down) {
                manager->linear_brightness_ambient = linear_brightness_ambient;
                /* Stop pending linear brightness */
...
set_backlight_brightness (GsdPowerManager *manager)
{
        ...
        /* Apply next brightness */
        if (counter < GSD_LINEAR_BRIGHTNESS_RAMP_DOWN) {
                manager->linear_brightness_ramp_id = g_timeout_add(
                                GSD_LINEAR_BRIGHTNESS_RAMP,
                                (GSourceFunc) set_backlight_brightness,
                                manager);
        /* Wait before applying next brightness */
        } else {
                counter = 0;
                manager->linear_brightness_ramp_id = g_timeout_add(
                                GSD_LINEAR_BRIGHTNESS_DELAY_DOWN,
                                (GSourceFunc) set_backlight_brightness,
                                manager);
        }
        return G_SOURCE_REMOVE;
...
static gint
get_ambient_for_brightness (GsdPowerManager *manager,
                            gint             brightness)
{
        g_autoptr (GVariantIter) iter = NULL;
        gint light_level;
        ...
        while (g_variant_iter_loop (iter, "{ii}", &light_level, &brightness_level)) {
                ...
                if (brightness <= brightness_level) {
                        ...
                        gint linear_brightness = brightness_level - lower_brightness_level;
                        return (
                                (brightness - lower_brightness_level) *
                                 linear_level / linear_brightness
                               ) + manager->linear_brightness_ambient_down;
                }
...
static void
set_ambient_normalize (GsdPowerManager *manager,
                       gint             brightness)
{
        ...
        if (manager->linear_brightness_points_fallback && brightness > 95) {
                ...
                gint normalize = get_ambient_for_brightness (manager, brightness);
                g_settings_set_int(manager->settings,
                                   "ambient-light-normalize",
                                   normalize - manager->linear_brightness_ambient_iio);
        }
}
                           ...
                           GError **error)
{
        ...
        if (g_strcmp0 (interface_name, GSD_POWER_DBUS_INTERFACE_SCREEN) == 0) {
                ...
                if (manager->backlight) {
                        set_ambient_normalize(manager, brightness_value);
